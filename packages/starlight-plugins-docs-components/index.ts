...
import { type StarlightPluginsDocsComponentsConfig, validateConfig, type StarlightPluginsDocsComponentsUserConfig } from "./libs/config";
...
export default function starlightPluginsDocsComponents(userConfig?: StarlightPluginsDocsComponentsUserConfig): StarlightPlugin {
    const config = validateConfig(userConfig);

    return {
        name: "@trueberryless-org/starlight-plugins-docs-components",
        hooks: {
            setup({ addIntegration, logger, config: starlightConfig, updateConfig: updateStarlightConfig }) {
                /**
                 ...
                 */
                logger.info("Hello from the @trueberryless-org/starlight-plugins-docs-components plugin!");
                    ...
                    hooks: {
                        "astro:config:setup": ({ injectRoute, updateConfig }) => {
                            injectRoute({
                                ...
                                entrypoint: "@trueberryless-org/starlight-plugins-docs-components/routes/Sites.astro",
                                prerender: true,
                            });

                            injectRoute({
                                pattern: "[...prefix]/resources/plugins",
                                entrypoint: "@trueberryless-org/starlight-plugins-docs-components/routes/Plugins.astro",
                                prerender: true,
                            });
                            ...
                            updateConfig({
                                vite: {
                                    plugins: [vitePluginStarlightPluginsDocsComponentsConfig(config)],
                                },
                            });
                        },
                    },
